...
courseSchema.pre('findOneAndDelete', async function (next) {
  try {
    ...
    const modulesWithLectures = await Module.find({
    ...
    if (modulesWithLectures.length > 0) {
      ...
      const allLectureIds = modulesWithLectures.flatMap((module) =>
        module.lectures.map((id) => id.toString()),
      )
      ...
      if (allLectureIds.length > 0) {
        ...
        if (lecturesToDelete.length > 0) {
          ...
          const publicIdsForVideoDeletion = lecturesToDelete.map(
    ...
    next(
      new AppError(
        `Error during course deletion cascade: ${error.message}`,
        500,
      ),
    )
  }
