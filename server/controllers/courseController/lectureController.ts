  ...
  try {
    ...
    if (req.file) {
      ...
      updates.url = {
        ...
        secureUrl: videoUploadResult.secure_url,
      }
      ...
      await Course.findByIdAndUpdate(course._id, {
        $inc: { totalOfHours: durationDifference },
      })
    }
...
// delete all lactures
export async function deleteallactures(
  req: Request,
  res: Response,
  next: NextFunction,
) {
...
export async function deleteLacture(
  ...
  try {
    ...
    if (
      !lecture.instructor ||
      lecture.instructor._id.toString() !== user._id.toString()
    ) {
      return next(
